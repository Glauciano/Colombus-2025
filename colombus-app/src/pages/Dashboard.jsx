...
import { Trophy, Truck, Wallet, Calendar, MapPin, TrendingUp } from 'lucide-react';
...
export default function Dashboard() {
  const provas = db.list(ENTITIES.PROVA);
  const custos = db.list(ENTITIES.CUSTO_LOGISTICO);
  const config = db.list(ENTITIES.CONFIGURACAO);
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard
          ...
          value={totalProvas}
          subtitle={`${copaFilhotes} Copa Filhotes · ${campeonatoAdultos} Campeonato`}
        />
        <StatCard
          icon={Truck}
          label="Gastos Logísticos"
          value={`R$ ${formatCurrency(totalCustos)}`}
          accent={totalCustos > 0}
        />
        <StatCard
          icon={Calendar}
          label="Provas Programadas"
          value={provas.filter(p => p.status === 'Programada').length}
        />
        <StatCard
          icon={MapPin}
          label="Cidades"
          value={[...new Set(provas.map(p => p.cidade).filter(Boolean))].length}
        />
      </div>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground">
                        {prova.categoria} · {prova.km ? `${prova.km} km` : '—'} · {formatDate(prova.data_solta)}
                      </p>
                    </div>
                  </div>
