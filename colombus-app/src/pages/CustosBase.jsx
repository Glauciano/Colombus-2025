...
function CustoForm({ custo, entity, onSave, onClose }) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (isEdit) {
      db.update(entity, custo.id, data);
    } else {
...
export default function CustosPage({ entity, title, subtitle, pdfName }) {
  ...
  const handleDelete = () => {
    db.delete(entity, deleteModal.custo.id);
  ...
  return (
    <div className="space-y-6">
            ...
            {sorted.map(custo => {
              ...
              return (
                <TableRow key={custo.id}>
                  <TableCell className="font-medium">{custo.cidade || '—'}</TableCell>
      ...
      <ConfirmDelete
        ...
        label={deleteModal.custo?.cidade}
      />
    </div>
  );
}
